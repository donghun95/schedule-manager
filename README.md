...
| POST   | `/api/schedules`                            | 일정 등록          | `{title,description,scheduledAt}`            | `201 + Location`                          | 로그인              |
| GET    | `/api/schedules`                            | 내 일정 목록        | `?status=&fromDate=&toDate=&cursor...`       | `200 + CursorResponse`                    | 로그인              |
| GET    | `/api/schedules/{id}`                       | 일정 단건 조회       | -                                            | `200 + ScheduleDetailResponse`            | OWNER / ACCEPTED |
| PATCH  | `/api/schedules/{id}`                       | 일정 수정          | `{title?,description?,scheduledAt?,version}` | `200 + ScheduleResponse`                  | OWNER            |
| PATCH  | `/api/schedules/{id}/status`                | 일정 상태 변경       | `{toStatus,version}`                         | `200 + ScheduleResponse`                  | OWNER            |
| DELETE | `/api/schedules/{id}`                       | 일정 삭제          | -                                            | `204`                                     | OWNER            |
| POST   | `/api/schedules/{id}/participants`          | 참여자 초대         | `{userId}`                                   | `201 + ScheduleParticipantResponse`       | OWNER            |
| GET    | `/api/schedules/{id}/participants`          | 참여자 목록 조회      | -                                            | `200 + List<ScheduleParticipantResponse>` | OWNER / ACCEPTED |
| DELETE | `/api/schedules/{id}/participants/{userId}` | 참여자 제거         | -                                            | `204`                                     | OWNER            |
| GET    | `/api/schedules/{id}/status-history`        | 일정 상태 변경 이력 조회 | -                                            | 상태 이력 목록                                  | OWNER / ACCEPTED |
| GET    | `/api/schedules/upcoming`                   | 다가오는 일정 조회     | `?size=10`                                   | `200 + List`                              | 로그인              |
| POST   | `/api/confirm-requests`                     | 결재 요청 등록       | `{scheduleId,approverId}`                    | `201 + ConfirmRequestResponse`            | 협업자              |
| PATCH  | `/api/confirm-requests/{id}/status`         | 결재 승인/반려       | `{status,feedback?}`                         | `200 + ConfirmRequestResponse`            | 담당자              |
| GET    | `/api/confirm-requests`                     | 결재 보관함         | `?type=RECEIVE/SEND&status=&size=&cursor...` | `200 + CursorResponse`                    | 로그인              |

### 관계 기반 접근 정책

| 사용자 관계   | 일정 단건 | 상태 이력 | 참여자 목록        | 일정 관리 |
| -------- | ----: | ----: | ------------- | ----: |
| OWNER    |    허용 |    허용 | **전체(ALL)**   |    허용 |
| ACCEPTED |    허용 |    허용 | **ACCEPTED만** |    거부 |
| PENDING  |    거부 |    거부 | 조회 불가         |    거부 |
| REJECTED |    거부 |    거부 | 조회 불가         |    거부 |
| NONE     |    거부 |    거부 | 조회 불가         |    거부 |
